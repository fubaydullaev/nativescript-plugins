import { StripeStandardAddress, IStripeStandardBackendAPI, StripeStandardConfig, StripeStandardCustomerSession, StripeStandardPaymentListener, StripeStandardPaymentSession, StripeStandardShippingAddressField, StripeStandardShippingMethod, StripeStandardPaymentMethodType } from '@fubaydullaev/nativescript-stripe/standard';
import { Page } from '@nativescript/core';
import { request } from '@nativescript/core/http';
import { Stripe, StripePaymentIntentParams, StripePaymentIntentStatus } from '@fubaydullaev/nativescript-stripe';
import { StripeRedirectSession } from '@fubaydullaev/nativescript-stripe';
	...
	capturePayment(stripeID: string, amount: number, shippingMethod?: StripeStandardShippingMethod, shippingAddress?: StripeStandardAddress): Promise<any> {
		...
		return new Promise(async (resolve, reject) => {
			...
			try {
				...
				if (global.isIOS) {
					pi.returnURL = 'iofubaydullaevnativescriptplugindemo://payment';
				} else {
					pi.returnURL = 'io.fubaydullaev.nativescript.plugindemo://payment';
				}
